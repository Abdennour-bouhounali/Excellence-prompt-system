...
## 5. HTML Formatting & Syntax Conversion Standards

1. **Zero Raw Markdown Syntax**: Final generated HTML must never display raw markdown formatting syntax (e.g., `<p>**valeur exacte**</p>` is strictly forbidden).
2. **Mandatory HTML Tag Conversion**: Convert all markdown formatting into semantic HTML tags:
   - `**text**` ➔ `<strong>text</strong>`
   - `*text*` or `_text_` ➔ `<em>text</em>`
3. **Non-Blocking Exercise Execution**: Exercise event handlers must never hard-block learning flow on wrong answers. Any completed student attempt MUST reveal diagnostic explanations and unlock the next section (`student_attempted ➔ unlock_next_stage()`).
